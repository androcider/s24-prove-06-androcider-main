...
public static class MapsTester {
    ...
    private static bool IsAnagram(string word1, string word2) {
        var wordmap1 =  new Dictionary<Char, int>();
        var wordmap2 =  new Dictionary<Char, int>();
        
        foreach (var letter in word1.Replace(" ", string.Empty).ToLower())
        {
            if (!wordmap1.ContainsKey(letter))
            {
                wordmap1[letter] = 1;
            }
            else
        ...
        foreach (var letter in word2.Replace(" ", string.Empty).ToLower())
        {
            if (!wordmap2.ContainsKey(letter))
            {
                wordmap2[letter] = 1;
            }
            else
        ...
        if (wordmap1.Count != wordmap2.Count)
        {
            return false;
        }

        foreach (var kvp in wordmap1)
        {
            if (!wordmap2.TryGetValue(kvp.Key, out int value))
            {
                return false;
            }
